...
router.get("/", auth, async (req, res) => {
  try {
    ...
    res.json({ name: user.name, theme: user.theme, bio: user.bio || "", avatar: user.avatar || "", isEmailVerified: true });
  } catch (err) {
...
router.post("/", auth, async (req, res) => {
  ...
  try {
    ...
    if (publicSlug) {
      const existing = await User.findOne({ publicSlug, _id: { $ne: user._id } });
...
// POST /api/settings/email/verify - resend email verification (mock)
